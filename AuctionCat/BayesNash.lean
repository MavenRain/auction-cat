...
theorem fpsb3_truthful_truthful_truthful_not_bayes_nash_n2 :
  ...
  have h_ge := h_br1 (fun _ => ⟨0, by decide⟩) ⟨1, by decide⟩
  have h_truth :
      fpsbExpectedUtility3 2 (fun v => v) (fun v => v) (fun v => v)
        ⟨1, by decide⟩ (fun v => if v.val = 0 then 1 else 0) = 0 := by
    unfold fpsbExpectedUtility3 fpsbUtility3
    native_decide
  have h_dev :
...
/-- **Pointwise nonneg of vickrey bidder-1 truthful utility val** (2 bidders). -/
theorem vickreyUtility_truthful_val_nonneg (n : Nat) (v b2 : Fin n) :
...
/-- **Pointwise bound**: vickrey3 bidder-2 utility is at most the
    valuation. -/
theorem vickreyBidder2Util3_val_le_valuation (n : Nat)
    (v opp_b1 my_b2 opp_b3 : Fin n) :
    (vickreyBidder2Util3 n v opp_b1 my_b2 opp_b3).val ≤ v.val := by
  unfold vickreyBidder2Util3
  by_cases h : opp_b1.val < my_b2.val ∧ my_b2.val ≥ opp_b3.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: fpsb3 bidder-2 utility is at most the
    valuation. -/
theorem fpsbBidder2Util3_val_le_valuation (n : Nat)
    (v opp_b1 my_b2 opp_b3 : Fin n) :
    (fpsbBidder2Util3 n v opp_b1 my_b2 opp_b3).val ≤ v.val := by
  unfold fpsbBidder2Util3
  by_cases h : opp_b1.val < my_b2.val ∧ my_b2.val ≥ opp_b3.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: vickrey3 bidder-3 utility is at most the
    valuation. -/
theorem vickreyBidder3Util3_val_le_valuation (n : Nat)
    (v opp_b1 opp_b2 my_b3 : Fin n) :
    (vickreyBidder3Util3 n v opp_b1 opp_b2 my_b3).val ≤ v.val := by
  unfold vickreyBidder3Util3
  by_cases h : opp_b1.val < my_b3.val ∧ opp_b2.val < my_b3.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: fpsb3 bidder-3 utility is at most the
    valuation. -/
theorem fpsbBidder3Util3_val_le_valuation (n : Nat)
    (v opp_b1 opp_b2 my_b3 : Fin n) :
    (fpsbBidder3Util3 n v opp_b1 opp_b2 my_b3).val ≤ v.val := by
  unfold fpsbBidder3Util3
  by_cases h : opp_b1.val < my_b3.val ∧ opp_b2.val < my_b3.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: fpsbReserve utility is at most the valuation. -/
theorem fpsbReserveUtility_val_le_valuation (n : Nat) (r v b1 b2 : Fin n) :
    (fpsbReserveUtility n r v b1 b2).val ≤ v.val := by
  unfold fpsbReserveUtility
  by_cases h : b1.val ≥ b2.val ∧ b1.val ≥ r.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: fpsbReserve bidder-2 utility is at most the
    valuation. -/
theorem fpsbReserveBidder2Util_val_le_valuation (n : Nat)
    (r v opp_bid my_bid : Fin n) :
    (fpsbReserveBidder2Util n r v opp_bid my_bid).val ≤ v.val := by
  unfold fpsbReserveBidder2Util
  by_cases h : opp_bid.val < my_bid.val ∧ my_bid.val ≥ r.val
  · simp [h]
  · simp [h]

/-- **Pointwise bound**: fpsbReserve3 bidder-1 utility is at most the
    valuation. -/
theorem fpsbReserveUtility3_val_le_valuation (n : Nat)
    (r v b1 b2 b3 : Fin n) :
    (fpsbReserveUtility3 n r v b1 b2 b3).val ≤ v.val := by
...
theorem vickreyBidder2Util_truthful_val_nonneg (n : Nat)
    (v opp_bid : Fin n) :
    0 ≤ (vickreyBidder2Util n v opp_bid v).val :=
  Nat.zero_le _

/-- **Pointwise nonneg of vickrey3 bidder-1 truthful utility val**. -/
theorem vickreyUtility3_truthful_val_nonneg (n : Nat) (v b2 b3 : Fin n) :
    0 ≤ (vickreyUtility3 n v v b2 b3).val :=
  Nat.zero_le _

/-- **Pointwise nonneg of vickrey3 bidder-2 truthful utility val**. -/
theorem vickreyBidder2Util3_truthful_val_nonneg (n : Nat)
    (v opp_b1 opp_b3 : Fin n) :
    0 ≤ (vickreyBidder2Util3 n v opp_b1 v opp_b3).val :=
  Nat.zero_le _

/-- **Pointwise nonneg of vickrey3 bidder-3 truthful utility val**. -/
theorem vickreyBidder3Util3_truthful_val_nonneg (n : Nat)
    (v opp_b1 opp_b2 : Fin n) :
    0 ≤ (vickreyBidder3Util3 n v opp_b1 opp_b2 v).val :=
  Nat.zero_le _

/-- **Bundled pointwise vickrey truthful nonneg utility vals** across
    all five bidder-position × bidder-count combinations. -/
theorem vickrey_truthful_val_nonneg_main (n : Nat) (v b2 b3 opp_bid : Fin n) :
    -- 2-bidder
    0 ≤ (vickreyUtility n v v b2).val
    ∧ 0 ≤ (vickreyBidder2Util n v opp_bid v).val
    -- 3-bidder
    ∧ 0 ≤ (vickreyUtility3 n v v b2 b3).val
    ∧ 0 ≤ (vickreyBidder2Util3 n v opp_bid v b3).val
    ∧ 0 ≤ (vickreyBidder3Util3 n v opp_bid b2 v).val :=
  ⟨vickreyUtility_truthful_val_nonneg n v b2,
   vickreyBidder2Util_truthful_val_nonneg n v opp_bid,
   vickreyUtility3_truthful_val_nonneg n v b2 b3,
