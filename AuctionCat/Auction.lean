...
/-- First-price auction with reserve `r` where bidder 1 uses strategy
    `bid` and bidder 2 is truthful. -/
def fpsbReserveAuctionDeviator1 (n : Nat) (r : Fin n)
    (bid : Fin n → Fin n) :
    StochasticMatrix (tensorObj n n) (tensorObj n n) :=
  auctionScoreDeviator1 n bid (fpsbReserve n r)

/-- First-price auction with reserve `r` where bidder 1 is truthful
    and bidder 2 uses strategy `bid`. -/
def fpsbReserveAuctionDeviator2 (n : Nat) (r : Fin n)
    (bid : Fin n → Fin n) :
    StochasticMatrix (tensorObj n n) (tensorObj n n) :=
  auctionScoreDeviator2 n bid (fpsbReserve n r)
...
/-- Vickrey auction with reserve price `r` where bidder 1 is truthful
    and bidder 2 uses strategy `bid`. -/
def spsbReserveAuctionDeviator2 (n : Nat) (r : Fin n) (bid : Fin n → Fin n) :
    StochasticMatrix (tensorObj n n) (tensorObj n n) :=
  auctionScoreDeviator2 n bid (spsbReserve n r)
...
/-- Three-bidder open game where bidder 2 uses strategy `bid` and
    bidders 1, 3 are truthful. -/
def auctionGame3Deviator2 (n : Nat) (bid : Fin n → Fin n) :
    OpenGame ((n * n) * n) ((n * n) * n) ((n * n) * n)
             (((2 * n) * (2 * n)) * (2 * n)) :=
  OpenGame.kron (auctionGameDeviator2 n bid) (truthfulBidder n)

/-- Score the three-bidder bidder-2-deviator open game against a
    mechanism. -/
def auctionScoreDeviator2_3 (n : Nat) (bid : Fin n → Fin n)
    (mech : StochasticMatrix (n * n * n) ((2 * n) * (2 * n) * (2 * n))) :
    StochasticMatrix ((n * n) * n) ((n * n) * n) :=
  OpenGame.score (auctionGame3Deviator2 n bid) mech

/-- Three-bidder Vickrey auction where bidder 2 uses strategy `bid`
    and bidders 1, 3 are truthful. -/
def spsb3AuctionDeviator2 (n : Nat) (bid : Fin n → Fin n) :
    StochasticMatrix ((n * n) * n) ((n * n) * n) :=
...
/-- Three-bidder Vickrey-with-reserve auction where bidder 2 uses
    strategy `bid` and bidders 1, 3 are truthful. -/
def spsb3ReserveAuctionDeviator2 (n : Nat) (r : Fin n) (bid : Fin n → Fin n) :
    StochasticMatrix ((n * n) * n) ((n * n) * n) :=
  auctionScoreDeviator2_3 n bid (spsb3Reserve n r)

/-- Three-bidder Vickrey-with-reserve auction where bidder 3 uses
    strategy `bid` and bidders 1, 2 are truthful. -/
def spsb3ReserveAuctionDeviator3 (n : Nat) (r : Fin n) (bid : Fin n → Fin n) :
    StochasticMatrix ((n * n) * n) ((n * n) * n) :=
  auctionScoreDeviator3_3 n bid (spsb3Reserve n r)
